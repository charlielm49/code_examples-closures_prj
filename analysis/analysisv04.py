...
columnas = ["IEBS", "COURSERA", "UNICEF", "ONU", "GOOGLE ADS", "FB", \
    "CIFAL", "MICROSOFT", "UBA"]
# Ordenamos para alinear bien las columnas y renglones en tabla final:
columnas.sort()
['CIFAL', 'COURSERA', 'FB', 'GOOGLE ADS', 'IEBS', 'MICROSOFT', 'ONU',
 'UBA', 'UNICEF']

# Contamos cuaántas alianzas de cada una tiene cada alumno por SEPARADO
a = data.groupby(['username', 'alsh']).count()['progmdesc']
'''
In [19]: a.sort_values()
Out[19]: 
username   alsh      
010000298  GOOGLE ADS    1
010475410  UNICEF        1
           GOOGLE ADS    1
           FB            1
010475401  GOOGLE ADS    1
                        ..
010429535  UNICEF        1
...
'''
# Convertimos en df y hacemos un registro por renglón
a = a.to_frame()
a.reset_index(inplace = True)
# Ponemos en formato alumno-alianza
a = a.pivot_table('progmdesc', 'username', 'alsh')
# 7855 rows - un renglón = info de 1 alumno único
'''
a
alsh       CIFAL  COURSERA  FB  GOOGLE ADS  IEBS  MICROSOFT  ONU  UBA  UNICEF
username                                                                     
010000298    NaN       NaN NaN         1.0   NaN        NaN  NaN  NaN     NaN
010001876    NaN       NaN NaN         NaN   NaN        NaN  NaN  NaN     1.0
010002693    NaN       NaN NaN         NaN   NaN        NaN  NaN  NaN     1.0
010003209    NaN       NaN NaN         NaN   NaN        NaN  NaN  NaN     1.0
...          ...       ...  ..         ...   ...  ...     ...
'''


#Obtenemos valores FUERA de la diagonal 
#la diag. sólo tiene el valor = 1, NO es conteo, el conteo es el count
# Los valores FUERA de la diag muestran cantidad de alianzas CRUZADAS
a.groupby("CIFAL", as_index=False).count()
Out[30]: 
alsh  CIFAL  COURSERA  FB  GOOGLE ADS  IEBS  MICROSOFT  ONU  UBA  UNICEF
0       1.0         0  12          12     0         10    0    3       1
        valor       conteo

b = pd.concat([a.groupby(x, as_index=False).count() for x in columnas])
b = b.fillna(0).astype(int)
b.index = columnas
# Mtriz con 1's en diagonal y simétrica
'''
alsh        CIFAL  COURSERA   FB  GOOGLE ADS  IEBS  MICROSOFT  ONU  UBA  UNICEF
CIFAL           1         0   12          12     0         10    0    3       1
COURSERA        0         1    0           0     0          0    0    0       0
FB             12         0    1         857     3         13    0    0      78
GOOGLE ADS     12         0  857           1     2         36    0    6     176
IEBS            0         0    3           2     1          0    0    2       2
MICROSOFT      10         0   13          36     0          1    0    1       6
ONU             0         0    0           0     0          0    1    0       0
UBA             3         0    0           6     2          1    0    1       4
UNICEF          1         0   78         176     2          6    0    4       1
'''

a.sum() # <- alumnos únicos TOT pro alianza (con o sin alianz cruzada)
# La suma del TOT es el tot de alianzas porque pueden estar en varias
a.sum().sum() = 9010

b.sum() # <- alumn únicos c/alianza cruzada (+ 1 de la diagonal artifi.)

# Esta diag Ya no sirve porque puiede haber alumnos con más de 2 alianz.
diag = a.sum() - (b.sum() - 1)
'''
diag
alsh
...
c = a[a["CIFAL"].notnull()].sum(axis=1)
username
010171042    2.0
010400167    2.0
010414971    2.0
...
len(list(filter(lambda x: (x == 1), c)))
4


pd.concat([a[x].notnull() for x in columnas])
[a[x].notnull() for x in columnas] ok
# t es la lista con todas las series de users que tienen datos y su suma
t = [a[a[x].notnull()].sum(axis = 1) for x in columnas]
[username
 010171042    2.0
 010400167    2.0
 010414971    2.0
 ...
 010418733    2.0
...
# Todos aquellos renglones que tengan la suma del renglón de a = 1 
# (significa que no tienen alianzas cruzadas)
#OK pero incompleto:
len(list(filter(lambda x: (x == 1), t[0])))
# Ciclo sobre la cantidad de elementos en lista columnas (alianzas)
diag = [len(list(filter(lambda x: (x == 1), t[y]))) \
...
for col_name in columnas:
    b.loc[col_name, col_name] = diag[count]
    count += 1
'''
alsh        CIFAL  COURSERA   FB  GOOGLE ADS  IEBS  MICROSOFT  ONU  UBA  UNICEF
CIFAL           4         0   12          12     0         10    0    3       1
COURSERA        0       747    0           0     0          0    0    0       0
FB             12         0   51         857     3         13    0    0      78
GOOGLE ADS     12         0  857         763     2         36    0    6     176
IEBS            0         0    3           2     8          0    0    2       2
MICROSOFT      10         0   13          36     0         19    0    1       6
ONU             0         0    0           0     0          0  253    0       0
UBA             3         0    0           6     2          1    0   17       4
UNICEF          1         0   78         176     2          6    0    4    4907
'''
