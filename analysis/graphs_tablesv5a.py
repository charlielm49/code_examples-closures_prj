...
#PATHBIN = "C:/users/HEIDY LORENA/miniconda3/envs/work/bin/"
#_______________________________________________________________________
#_______________________________________________________________________
...
if upload is not None:
    f = pd.ExcelFile(upload)
    archList = f.sheet_names
    #OK print("\n\n", archList)
    tmpList = []
    ...
    data.columns = ['username', 'fecha', 'primer_inscrip', \
        'nivel', 'proglong', 'etiqueta', 'modal', 'alsh']
    print("todos concat DESPUES:\n", data)
    # Guardamos archivo
    data.to_csv(PATHP + "data/data1.csv", index = False)
    # Ejecutamos proceso de archivo
    subprocess.call(PATHBIN+"python "+PATHP+"analysis/analysisv04a.py", \
        shell = True)
